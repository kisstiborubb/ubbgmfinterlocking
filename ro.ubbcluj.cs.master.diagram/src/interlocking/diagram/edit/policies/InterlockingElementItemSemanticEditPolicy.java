 ...
 * @generated
 */
public class InterlockingElementItemSemanticEditPolicy extends
		InterlockingBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public InterlockingElementItemSemanticEditPolicy() {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (InterlockingVisualIDRegistry.getVisualID(node)) {
			case InterlockingElementInterlockingElementConnectorsCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (InterlockingVisualIDRegistry.getVisualID(cnode)) {
					case ConnectorEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (InterlockingVisualIDRegistry
									.getVisualID(incomingLink) == InterlockingConnectionEditPart.VISUAL_ID) {
						...
						for (Iterator<?> it = cnode.getSourceEdges().iterator(); it
							...
							if (InterlockingVisualIDRegistry
									.getVisualID(outgoingLink) == InterlockingConnectionEditPart.VISUAL_ID) {
								DestroyElementRequest r = new DestroyElementRequest(
